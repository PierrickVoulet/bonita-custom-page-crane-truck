...
/** temp task to rename img resource path after building dist */
gulp.task('repathimg', ['repath'], function () {
  return gulp.src('target/dist/index.html')
    .pipe(plumber())
    .pipe(replace(/(src=["|']([^"']*\.jpg)["|'])/g, 'src="pageResource?page=' + customPageName + '&location=$2"'))
    .pipe(gulp.dest('target/dist'));
});

gulp.task('assets', ['assetsimg', 'assetslib'], function () {
...
gulp.task('assetsimg', function () {
  return gulp.src('src/img/*.jpg')
    .pipe(gulp.dest('target/dist/resources/img'));
});

gulp.task('assetslib', function () {
  return gulp.src('src/lib/*.jar')
...
gulp.task('zip', ['assets', 'repathimg'], function (done) {
